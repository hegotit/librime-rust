use crate::rime::key_table::{
    rime_get_key_name, rime_get_keycode_by_name, rime_get_modifier_by_name, rime_get_modifier_name,
    Mask, ALT_MASK, XK_VOID_SYMBOL,
};
use log::error;
...
impl KeyEvent {
    fn new(keycode: u32, modifier: i32) -> Self {
    ...
    fn alt(&self) -> bool {
        (self.modifier & (ALT_MASK as i32)) != 0
    }
    ...
    pub fn repr(&self) -> String {
        ...
        if self.modifier != 0 {
            ...
            while k != 0 {
                if (k & 1) != 0 {
                    if let Some(modifier_name) = rime_get_modifier_name(1 << i) {
        ...
        if let Some(name) = rime_get_key_name(self.keycode as u32) {
    ...
    pub fn parse(&mut self, repr: &str) -> bool {
        ...
        for &token in &tokens[..tokens.len() - 1] {
            let mask = rime_get_modifier_by_name(Some(token));
            if mask != 0 {
                self.modifier |= mask;
            } else {
        ...
        self.keycode = rime_get_keycode_by_name(key_token);
