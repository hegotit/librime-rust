...
type Notifier = Signal<(Context,)>;
type OptionUpdateNotifier = Signal<(Context, String)>;
type PropertyUpdateNotifier = Signal<(Context, String)>;
type KeyEventNotifier = Signal<(Context,)>;
...
impl Context {
    ...
    fn commit(&mut self) -> bool {
        ...
        self.commit_notifier.emit(self.clone());
        self.clear();
        true
    }
    ...
    fn has_menu(&self) -> bool {
        ...
        if let Some(menu) = &self.composition.0.segments.last().unwrap().menu {
    ...
    fn get_selected_candidate(&self) -> Option<Arc<dyn Candidate>> {
        ...
        self.composition.0.segments.last()?.get_selected_candidate()
    }
    ...
    fn clear(&mut self) {
        self.input.clear();
        self.caret_pos = 0;
        self.composition.0.segments.clear();
        self.update_notifier.emit(self.clone());
    }
    ...
    fn select(&mut self, index: usize) -> bool {
        ...
        let seg = self.composition.0.segments.last_mut();
        if let Some(seg) = seg {
    ...
    fn highlight(&mut self, index: usize) -> bool {
        if self.composition.is_empty() || self.composition.0.segments.last().unwrap().menu.is_none()
        {
            return false;
        }
        let seg = self.composition.0.segments.last_mut().unwrap();
    ...
    fn delete_candidate_by_index(&mut self, index: usize) -> bool {
        if let Some(seg) = self.composition.0.segments.last() {
    ...
    fn confirm_current_selection(&mut self) -> bool {
        ...
        if let Some(seg) = self.composition.0.segments.last_mut() {
            seg.status = SegmentStatus::Selected;
    ...
    fn begin_editing(&mut self) {
        for seg in self.composition.0.segments.iter_mut().rev() {
    ...
    fn clear_previous_segment(&mut self) -> bool {
        if let Some(last_segment) = self.composition.0.segments.last() {
    ...
    fn reopen_previous_selection(&mut self) -> bool {
        let len = self.composition.0.segments.len();
        let mut target_index = None;

        for i in (0..len).rev() {
            let seg = &mut self.composition.0.segments[i];
            if seg.status > SegmentStatus::Selected {
                return false;
            }
            if seg.status == SegmentStatus::Selected {
        ...
        if let Some(index) = target_index {
            self.composition.0.segments.truncate(index + 1);
            self.composition.0.segments[index].reopen(self.caret_pos);
            self.update_notifier.emit(self.clone());
            return true;
        }
    ...
    fn clear_non_confirmed_composition(&mut self) -> bool {
        let reverted = if let Some(pos) = self
            .composition
            .0
            .segments
            .iter()
        ...
        {
            self.composition.0.segments.truncate(pos + 1);
            true
        } else if !self.composition.0.segments.is_empty() {
            self.composition.0.segments.clear();
            true
        } else {
            false
