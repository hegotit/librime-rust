...
impl ConfigValue {
    ...
    fn parse_int(&self) -> Option<i32> {
        ...
        if self.value.starts_with("0x") {
            if let Ok(hex) = i32::from_str_radix(&self.value[2..], 16) {
                return Some(hex);
            }
        }
    ...
    fn parse_double(&self) -> Option<f64> {
        if self.value.is_empty() {
            return None;
        }
...
impl ConfigList {
    ...
    fn append(&mut self, element: Arc<dyn ConfigItem>) {
        self.seq.push(Some(element));
    }
...
impl ConfigMap {
    ...
    fn get(&self, key: &str) -> Option<Arc<dyn ConfigItem>> {
        self.map.get(key).cloned()
    }

    fn get_value(&self, key: &str) -> Option<Arc<ConfigValue>> {
