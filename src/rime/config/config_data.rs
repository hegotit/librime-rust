...
impl ConfigData {
    ...
    pub(crate) fn save_to_file(&mut self, file_path: &PathBuf) -> bool {
        self.modified = false;
    ...
    pub(crate) fn save_to_stream(&self, stream: &mut dyn Write) -> bool {
    ...
    pub(crate) fn load_from_file(&mut self, file_path: &Path, _compiler: &ConfigCompiler) -> bool {
        // Update status
    ...
    fn read_and_parse_file(&self, file_path: &Path) -> Result<Arc<dyn ConfigItem>, String> {
        let contents = self.read_file(file_path).map_err(|e| e.to_string())?;
        self.parse_yaml(&contents)
    }

    fn read_file(&self, file_path: &Path) -> Result<String, Error> {
