...
pub struct Calculus {
    factories: HashMap<String, Box<dyn Fn(Vec<String>) -> Option<Arc<dyn Calculation>>>>,
}
...
impl Calculus {
    ...
    pub fn register(
        &mut self,
        token: &str,
        factory: Box<dyn Fn(Vec<String>) -> Option<Arc<dyn Calculation>>>,
    ) {
        self.factories.insert(token.to_string(), factory);
    }

    pub fn parse(&self, definition: &str) -> Option<Arc<dyn Calculation>> {
