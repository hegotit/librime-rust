use crate::rime::config::config_types::{ConfigMap, ConfigValue};
use crate::rime::config::{config_component::Config, config_types::ConfigList};
...
impl TableEncoder {
    ...
    pub(crate) fn load_settings(&mut self, config: Option<&Config>) -> bool {
        ...
        if let Some(excludes) = config.get_list("encoder/exclude_patterns") {
            for pattern in excludes.seq.iter().flatten() {
                let Some(pattern) = pattern.as_any().downcast_ref::<ConfigValue>() else {
                    continue;
                };
                if let Ok(regex) = Regex::new(pattern.str()) {
    ...
    fn process_rules(&mut self, config: &Config) {
        if let Some(rules) = config.get_list("encoder/rules") {
            for rule in rules.seq.iter().flatten() {
                if let Some(rule) = rule.as_any().downcast_ref::<ConfigMap>() {
                    self.process_rule(rule);
                }
            }

            if self.max_phrase_length > MAX_PHRASE_LENGTH {
    ...
    fn process_rule(&mut self, rule: &ConfigMap) {
        if let Some(formula) = rule.get_value("formula") {
            let formula = formula.str();
            if !formula.is_empty() {
    ...
    fn set_lengths(&mut self, rule: &ConfigMap, table_rule: &mut TableEncodingRule) -> bool {
        if let Some(value) = rule.get_value("length_equal") {
            if let Some(length) = value.parse_int() {
    ...
    fn dfs_encode(
        ...
        if start_pos == phrase.len() {
            *limit -= 1;
            ...
            if let Some(encoded) = self.encode(code) {
                ...
                self.encoder.collector.create_entry(phrase, &encoded, value);
                return true;
            } else {
        ...
        let translations = self.encoder.collector.translate_word(word);
...
impl ScriptEncoder {
    ...
    fn dfs_encode(
        ...
        for k in (1..=phrase.len() - start_pos).rev() {
            ...
            if let Some(translations) = self.encoder.collector.translate_word(word) {
                for translation in translations {
                    code.0.push(translation);
