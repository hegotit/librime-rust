...
impl Script {
    ...
    pub fn merge(&mut self, s: &str, sp: &SpellingProperties, v: Vec<Spelling>) {
    ...
    pub fn dump(&self, file_path: &Path) -> Result<()> {
...
impl Projection {
    ...
    pub fn load(&mut self, settings: Option<Arc<ConfigList>>) -> bool {
        ...
        for (i, _) in settings.seq.iter().enumerate() {
            ...
            match calc.parse(formula) {
                Some(x) => self.calculation.push(x),
                None => {
                    error!(
    ...
    pub fn apply_script(&self, value: Option<&mut Script>) -> bool {
        let Some(value) = value else {
            return false;
        };
        ...
        for calc in &self.calculation {
            ...
            for (key, vec_spelling) in &value.map {
                ...
                let mut applied = calc.apply(Some(&mut s));
