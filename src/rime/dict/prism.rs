...
impl Prism {
    ...
    pub fn expand_search(&self, key: &str, limit: usize) -> Option<Vec<Match>> {
        ...
        let alphabet_iter: Box<dyn Iterator<Item = char>> = match &self.metadata {
            Some(metadata) if self.format > 1.0 - f64::EPSILON => {
                Box::new(metadata.alphabet.iter().copied())
            }
            _ => Box::new(DEFAULT_ALPHABET.chars()),
        };
        ...
        for (index, ch) in alphabet_iter.enumerate() {
            if index < limit {
                let new_key = format!("{}{}", key, ch);
        ...
        let result = queue
            .into_iter()
            .filter_map(|query_key| trie.common_prefix_search(&query_key))
            .flat_map(|list| list.into_iter().map(Match::from))
            .collect();
