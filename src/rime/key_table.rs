...
static KEY_NAMES: LazyLock<&[u8]> = LazyLock::new(|| {
    b"space\0\
    exclam\0\
    quotedbl\0\
    numbersign\0\
...
pub fn get_modifier_name(mut modifier: u32) -> Option<&'static str> {
...
pub fn get_keycode_by_name(name: &str) -> u32 {
    find_key_mapping(KEYS_BY_KEYVAL.deref(), |&(key_value, offset)| {
        extract_str_from_offset(offset, KEY_NAMES.deref())
            .filter(|&key_name| key_name == name)
            .map(|_| key_value)
    })
    .unwrap_or(XK_VOID_SYMBOL)
}

pub fn get_key_name(keycode: u32) -> Option<&'static str> {
    find_key_mapping(KEYS_BY_NAME.deref(), |&(key_value, offset)| {
        if key_value == keycode {
            extract_str_from_offset(offset, KEY_NAMES.deref())
        } else {
            None
        }
...
fn find_key_mapping<F, T>(mappings: &[(u32, usize)], predicate: F) -> Option<T>
where
    F: Fn(&(u32, usize)) -> Option<T>,
{
    mappings.iter().find_map(predicate)
}

fn extract_str_from_offset(offset: usize, key_names: &[u8]) -> Option<&str> {
    let slice = &key_names[offset..];
